...
def freeze_bn(obj, layers, models):
    for layer in obj.layers:
        ...
        elif isinstance(layer, models.Model):
            freeze_bn(layer, layers, models)


def mult_dropout_rate(obj, coeff, layers, models):
    for layer in obj.layers:
        # todo: As can be found in EfficientNet sources, layer Dropout should be fixed for multi-version keras
        if isinstance(layer, layers.Dropout):
            ...
            mult_dropout_rate(layer, coeff, layers, models)
...
def get_solver(**kwarguments):
    ...
    class SegmSolver(ISolver):
        def __init__(self, conf):
            ...
            # Practice hints:
            # 'focal_loss' could maximize F1, but it has no any affect to F2 (even for train-set) when dataset extended 
            # by "rich" background samples. Even 0.001 value of 'focal_loss' could make 0.8 F1 and 0.1 F2.
            # If add 'dice_loss', total loss will grown from 0.001 (focal_loss only) to 0.9. F2 became Increases with 
        ...
        def _create(self, compile_model=True, **kwargs):
            ...
            if hasattr(self.conf, 'dropout_rate_mult'):
                if 0.0 <= self.conf.dropout_rate_mult != 1:
                    mult_dropout_rate(self.model, self.conf.dropout_rate_mult, _layers, _models)
                    logging.info('Dropout layers have been modified by coefficient {}'.
                                 format(self.conf.dropout_rate_mult))
            ...
            if weights_init_path is not None:
                ...
                if hasattr(self.conf, 'freeze_bn'):
                    if self.conf.freeze_bn:
                        freeze_bn(self.model, _layers, _models)
            ...
            if compile_model:
                ...
                if weights_init_path is not None and isinstance(self.conf.optimizer, _optimizers.Adam):
